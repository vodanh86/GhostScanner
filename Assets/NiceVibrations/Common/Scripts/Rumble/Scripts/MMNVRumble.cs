...
namespace MoreMountains.NiceVibrations
{
    public static class MMNVRumble 
    {
        ...
        public static void Rumble(float lowFrequency, float highFrequency, float duration, MonoBehaviour coroutineSupport)
        {
            coroutineSupport.StartCoroutine(RumbleCoroutine(lowFrequency, highFrequency, duration));
        }
        ...
        {
            coroutineSupport.StartCoroutine(RumblePatternCoroutine(pattern, amplitudes, amplitudes, repeat, coroutineSupport));
        }
        ...
        public static void Rumble(long[] pattern, int[] lowFreqAmplitudes, int[] highFreqAmplitudes, int repeat, MonoBehaviour coroutineSupport)
        {
        ...
        /// <param name="repeat"></param>
        /// <returns></returns>
        private static IEnumerator RumblePatternCoroutine(long[] pattern, int[] lowFreqAmplitudes, int[] highFreqAmplitudes, int repeat, MonoBehaviour coroutineSupport)
        {
            float startedAt = Time.unscaledTime;
            float duration = 0f;
            for (int i = 0; i < pattern.Length; i++)
            {
                ...
                Gamepad.current.SetMotorSpeeds(lowFreqAmplitude, highFreqAmplitude);
        ...
        /// <param name="lowFrequency"></param>
        /// <param name="highFrequency"></param>
        public static void RumbleContinuous(float lowFrequency, float highFrequency)
        {
            if (Gamepad.current == null)
            {
                return;
