...
Shader "Vortex Game Studios/Filters/OLD TV Filter/Noise" {
	Properties {
		...
		_PatternScaleX("Noise Scale X", float) = 1.0
		_PatternScaleY("Noise Scale Y", float) = 1.0
	}
	...
	SubShader {
		Pass {
			...
			half4 frag(v2f i) : SV_Target{
				_PatternOffsetX = _SinTime.w * 15 * _PatternScaleX;
				_PatternOffsetY = _PatternOffsetX * 20 * _PatternScaleY;

				half4 output = tex2D(_MainTex, i.uv);
				half3 noise = tex2D(_PatternTex, half2(_PatternOffsetX, _PatternOffsetY) + i.uv * half2(_PatternScaleX, _PatternScaleY)).rgb;
				//half3 noise = tex2D(_PatternTex, half2(_PatternOffsetX, _PatternOffsetY) + i.uv * half2(_PatternScaleX, _PatternScaleY)).rgb;
